...
/** Matches and captures base JIRA issue URL and ticket ID */
export const JIRA_URL_REGEX = /^(https:\/\/jira\.ethanjohnsrud\.com)\/(?:browse\/|jira\/software\/c\/projects\/[^\/?#]+\/boards\/\d+\/backlog\?selectedIssue=)(?<jiraCode>[A-Z]{2,5}-\d{2,5})$/;
...
export const AGO_URL_REGEX = /^https:\/\/(?<region>[a-zA-Z]{2,7})(?:[-.](?<environment>[a-z]+)[a-z.]+)\/(?<route>advisergo)\/#\/(?<clientID>[a-f0-9]{27}[a-f0-9]{5})\/(?<planID>[a-f0-9]{27}[a-f0-9]{5})/;
