...
export default function UrlItemEditor({ isOpen, onOpenChange, storageListKey, urlItem }: UrlItemEditorProps) {
	...
	useEffect(() => {
		...
		const timeout = window.setTimeout(() => {
			const itemIndex = storageList.findIndex((item) => item.id === lookupIdRef.current || item.url === lookupUrlRef.current);
			if (itemIndex === -1) {
				setSaveState("idle");
				return;
			}
	...
	const handleDelete = () => {
		...
		const updatedList = storageList.filter((item) => item.id !== lookupIdRef.current && item.url !== lookupUrlRef.current);
		saveToStorage({ [storageListKey]: updatedList });
		overlayState.close();
	};
					...
					<Modal.Dialog className="overflow-hidden">
						<Modal.Header>
							...
							<div className="flex items-center justify-between gap-3">
								<Chip color="accent" size="sm">{draft.type}</Chip>
