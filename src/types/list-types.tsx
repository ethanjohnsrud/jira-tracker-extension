...
export const validateJiraList = (objectList:Object[], debugMode:boolean):boolean => {
    ...
    for(const item of list) {
        try {
            if(!item.id || !item.url || !item.originalUrl || !item.displayName || typeof item.lastVisitedMS !== 'number' ||
                !item.jiraCode || !item.title || !item.sprint || !item.status ||
                (list.filter((listItem) => listItem?.url === item.url).length > 1)
            ) throw new Error('Invalid JiraListItem.');

            if(item.targetDateMS !== undefined && typeof item.targetDateMS !== 'number') throw new Error('Invalid JiraListItem.targetDateMS.');
            if(item.preserveCustomName !== undefined && typeof item.preserveCustomName !== 'boolean') throw new Error('Invalid JiraListItem.preserveCustomName.');
            if(item.favorite !== undefined && typeof item.favorite !== 'boolean') throw new Error('Invalid JiraListItem.favorite.');
            if(item.collectionName !== undefined && !item.collectionName) throw new Error('Invalid JiraListItem.collectionName.');
...
export const validateAGOList = (objectList:Object[], debugMode:boolean):boolean => {
    ...
    for(const item of list) {
        try {
            if(!item.id || !item.url || !item.originalUrl || !item.displayName || typeof item.lastVisitedMS !== 'number' ||
                !item.region || !item.environment || !item.route || !item.planName || !item.clientFullName || !item.clientLastName ||
                (list.filter((listItem) => listItem?.url === item.url).length > 1)
            ) throw new Error('Invalid AGOListItem.');

            if(item.preserveCustomName !== undefined && typeof item.preserveCustomName !== 'boolean') throw new Error('Invalid AGOListItem.preserveCustomName.');
            if(item.favorite !== undefined && typeof item.favorite !== 'boolean') throw new Error('Invalid AGOListItem.favorite.');
            if(item.collectionName !== undefined && !item.collectionName) throw new Error('Invalid AGOListItem.collectionName.');
